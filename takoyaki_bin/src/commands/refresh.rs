...
pub fn refresh() {
    ...
    for name in std::fs::read_dir(config_directory.join("plugins")).unwrap_or_else(|_| { logger.error("Cannot find the directory! Make sure you have ran `takoyaki init`") }) {
        ...
        std::fs::remove_dir_all(config_directory.join("cache").join(&file_name))
            .unwrap_or_else(|_| { no_dir_error(&logger) });
